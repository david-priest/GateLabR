...
- The embedded GateLab core is GateLab-dev master at 4375b3f (0.7.5 plus the pooled gate label, memberships on explicit save, and Colour by colData), which sends the memberships with an explicit save and, when the plot pools several checked files, labels gate counts pooled over the same files.
