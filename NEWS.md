...
- The bundled GateLab interface picks up elliptical gates. An ellipse is dragged out from its
  centre and carries four handles at its axis ends. Dragging a handle sets that axis and turns
  the ellipse to follow the cursor, so rotation needs no separate control. Ellipses are stored
  the way Gating-ML stores them, as a mean, a covariance matrix and the squared distance its
  boundary sits at, and membership is evaluated from those numbers rather than from a sampled
  outline, so an ellipse stays an ellipse however the axes are displayed.
- Fixed: a workspace holding an elliptical gate aborted the SCE autosave, reporting that the
  gate had invalid vertices. An ellipse has no vertices, but the workspace mirror required them
  of every gate that was not a quadrant. The mirror now records the ellipse parameters and adds
  a sampled boundary alongside them, so anything reading vertices still receives the correct
  geometry while membership stays defined by the covariance.
- Gates that belong to no population now stay visible, controlled by an Unowned gates checkbox
  beside Branch gates. Such a gate sits in no branch, so branch scoping could only hide it by
  accident, and a gate drawn without creating a population vanished as soon as anything else was
  selected.
- Fixed: a polygon with a repeated vertex selected every event in its bounding box. The repeated
  point produced a zero-length edge, which the crossing test read as lying on the boundary, so
  the gate quietly reported far more events than it contained. Any polygon whose outline had
  been densified and re-imported was affected.
- BD FACSDiva experiment XML can be imported, bringing across the gate tree, the tube
  compensation matrices and Diva's biexponential display, which is a Logicle in disguise.
- Fixed: CyTOF Gaussian channels such as Width kept their declared arcsinh space on Gating-ML
  import, instead of being treated as raw and landing far below the data.
- Fixed: dragging a gate that was not already selected no longer snaps it back to where it
  started, so moving a gate no longer takes two attempts.
- Cytobank Gating-ML export carries the compensation matrix as a spectrumMatrix block, derives
  each channel's scale range from the data rather than assuming one, and collapses the extra
  vertices added when a curved gate edge is densified.
- The R host protocol is unchanged. All six host contracts remain at version 1, so saved
  workspaces, compensation provenance and assay bindings load without conversion.
