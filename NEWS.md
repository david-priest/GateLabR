...
- `gatelabImportGate()` puts a gate whose geometry is already known into the workspace on an SCE, and creates a population for it. Until now the app was the only way in, which is fine while every gate is new and stops being fine when a gate already exists and must not change: a gate behind a published figure, one drawn before this package did, or one arriving from a FlowJo workspace. Those have coordinates and nothing to put them in. The channels travel with the geometry, which is the point — coordinates alone can be replotted against any pair of channels and will still look like a gate, whereas a workspace gate carries `x_channel` and `y_channel` and is checked against the SCE.
- Import writes geometry and **nothing else**. Which events fall inside a gate is settled in the browser, per gate space and transform, and a second engine in R could silently disagree, so the stored memberships are left untouched and the workspace revision is bumped past them. `gatelabPopulations()` then refuses the stale mask with the message it already has, and one "Save to SCE" brings them back in step with the only gating engine there is.
- The import validates through the app's own door rather than a second set of rules: the assembled workspace is re-checked for format, sample identities against the SCE, compensation bindings, the whole gating graph and every channel before it is stored, so a refused import leaves the object untouched. It refuses a channel the SCE does not have, naming near misses; a polygon with fewer than three vertices or a rectangle without exactly two corners; and a duplicate gate name unless `overwrite = TRUE`, because a gate is the record of a decision. Importing into an SCE with no workspace at all is refused with instructions rather than synthesised, since the sample identities and assay bindings a workspace must agree with are the app's to write.
