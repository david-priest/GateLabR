# Authors and Citation

## Authors

- **David Priest**. Author, maintainer.
  [](https://orcid.org/0000-0003-0095-6718)

## Citation

Source:
[`DESCRIPTION`](https://github.com/david-priest/GateLabR/blob/v1.2.1/DESCRIPTION)

Priest D (2026). *GateLabR: Interactive Manual Gating for
SingleCellExperiment Objects*. R package version 1.2.1,
<https://david-priest.github.io/GateLabR>.

    @Manual{,
      title = {GateLabR: Interactive Manual Gating for SingleCellExperiment Objects},
      author = {David Priest},
      year = {2026},
      note = {R package version 1.2.1},
      url = {https://david-priest.github.io/GateLabR},
    }
